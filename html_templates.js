...
<fieldset class="pb-3 contentWidth d-flex flex-column align-items-center">
    <button type="button" class="btn w-75 px-5 py-3 mx-2 my-2 btn-secondary " id="typeSetting" onclick="toggleType()">Quiz-Type: Flags -> Country</button>
...
function answerTemplate(x) {
    let answers = "";
    for (let i = 0; i < 4; i++) {
     answers += /*html*/`<button type="button" class="btn contentWidth px-5 py-3 mx-2 my-2 btn-outline-primary">${x[i]}</button>`;
    };
    return answers;
}
